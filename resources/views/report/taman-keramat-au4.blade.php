<x-app-layout>
  <x-slot name="title">
    {{ __('Taman Keramat AU4') }}
  </x-slot>

  <div class="flex flex-col w-full px-6 py-8 gap-y-6">
    <div class="flex flex-col p-4 bg-white shadow sm:p-8 sm:rounded gap-y-4">
      <div class="flex flex-col w-full">
        <h1 class="text-xl font-bold">Taman Keramat AU4</h1>
        <p class="text-sm leading-relaxed">Taman Keramat AU4 student's details.</p>
      </div>
        ...
        <table class="w-full text-sm text-center whitespace-no-wrap table-auto">
          ...
          <tbody>
            ...
            @foreach ($allStudent as $allStudentData)
              @if (
                  $allStudentData->pickup_address->area === 'Taman Keramat AU4' ||
                      $allStudentData->dropoff_address->area === 'Taman Keramat AU4')
      ...
      <div class="flex items-center p-2 mt-2 gap-x-4">
        <a class="px-8 py-2 text-sm text-white bg-[#08183A] rounded w-max focus:outline-none hover:bg-[#08183A]/[.8]"
          href="{{ route('report.export-taman-keramat-au4') }}">
          Export to Excel
        </a>
