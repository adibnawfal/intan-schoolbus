<form method="post" action="{{ route('profile.delete-parent-guardian', $parentGuardianData->id) }}">
  @csrf
  @method('delete')

  <div id="hs-delete-parent-guardian-{{ $parentGuardianData->id }}-modal"
      ...
      <div
        ...
        <div class="relative flex flex-col h-full p-6 overflow-hidden rounded">
          <h1 class="flex items-center pb-4 mb-4 text-4xl leading-none text-gray-900 border-b border-gray-200">
            <span>Delete {{ $parentGuardianData->status }}</span>
          </h1>
          <div class="w-full">
            <p>Are you sure you want to delete {{ $parentGuardianData->first_name }}
              {{ $parentGuardianData->last_name }} profile?</p>
          </div>
          <button type="submit"
