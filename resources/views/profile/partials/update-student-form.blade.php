...
<form method="post" action="{{ route('profile.patch-update-student', $studentData->id) }}">
  @csrf
  @method('patch')
  ...
  <div class="flex flex-wrap -m-2">
    <div class="w-1/2 p-2">
      ...
      <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $studentData->first_name) }}"
    ...
    <div class="w-1/2 p-2">
      ...
      <div class="relative">
        <select id="standard" name="standard"
          class="w-full px-3 py-1 text-base leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-gray-100 bg-opacity-50 border border-gray-300 rounded outline-none focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200">
          <option @selected(old('standard', $studentData->standard) == null) value=null disabled>Select your standard</option>
          <option @selected(old('standard', $studentData->standard) == 1) value=1>1</option>
          <option @selected(old('standard', $studentData->standard) == 2) value=2>2</option>
          <option @selected(old('standard', $studentData->standard) == 3) value=3>3</option>
          <option @selected(old('standard', $studentData->standard) == 4) value=4>4</option>
          <option @selected(old('standard', $studentData->standard) == 5) value=5>5</option>
          <option @selected(old('standard', $studentData->standard) == 6) value=6>6</option>
        </select>
      </div>
    ...
    <div class="w-1/2 p-2">
      ...
      <div class="relative">
        <select id="gender" name="gender"
          class="w-full px-3 py-1 text-base leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-gray-100 bg-opacity-50 border border-gray-300 rounded outline-none focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200">
          <option @selected(old('gender', $studentData->gender) == '') value="" disabled>Select your gender</option>
          <option @selected(old('gender', $studentData->gender) == 'Male') value="Male">Male</option>
          <option @selected(old('gender', $studentData->gender) == 'Female') value="Female">Female</option>
        </select>
      </div>
  ...
  <div class="w-full mt-2">
    <select id="parent_guardian_id" name="parent_guardian_id"
      ...
      <option @selected(old('parent_guardian_id', $studentData->parent_guardian_id) == null) value=null disabled>Select Parent/Guardian Information</option>
      @foreach ($parentGuardian as $parentGuardianData)
        <option @selected(old('parent_guardian_id', $studentData->parent_guardian_id) == $parentGuardianData->id) value={{ $parentGuardianData->id }}>
  ...
  <div class="w-full mt-2">
    <select id="pickup_address_id" name="pickup_address_id"
      class="w-full px-8 py-3 mt-2 font-semibold border border-gray-300 rounded hover:shadow">
      <option @selected(old('pickup_address_id', $studentData->pickup_address_id) == null) value=null disabled>Select Pick-Up Address</option>
      @foreach ($address as $addressData)
        <option @selected(old('pickup_address_id', $studentData->pickup_address_id) == $addressData->id) value={{ $addressData->id }}>
  ...
  <div class="w-full mt-2">
    <select id="dropoff_address_id" name="dropoff_address_id"
      class="w-full px-8 py-3 mt-2 font-semibold border border-gray-300 rounded hover:shadow">
      <option @selected(old('dropoff_address_id', $studentData->dropoff_address_id) == null) value=null disabled>Select Drop-Off Address</option>
      @foreach ($address as $addressData)
        <option @selected(old('dropoff_address_id', $studentData->dropoff_address_id) == $addressData->id) value={{ $addressData->id }}>
          {{ $addressData->address_1 }}, {{ $addressData->address_2 }}, {{ $addressData->postal_code }}
          {{ $addressData->city }}, {{ $addressData->state }} - ({{ $addressData->area }})
        </option>
      @endforeach
